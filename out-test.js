...
var toolsFunctions = {
  querySemakmuleDB: async ({ phoneNumber, bankAccount }) => {
  ...
  draftPoliceReport: async ({ incidentDetails, scammerContact, financialLoss }) => {
    console.log(`[Agent Tool] Executing draftPoliceReport for ${scammerContact}`);
    await new Promise((resolve) => setTimeout(resolve, 1e3));
    const timestamp = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
    const reportRef = `NSRC-${Math.floor(Math.random() * 9e4) + 1e4}`;
    const draft = `
...
async function runAgenticChat(chatHistory) {
  ...
  const model = genAI.getGenerativeModel({
    ...
    systemInstruction: "You are the ScamShield MY AI Advisor, a helpful autonomous assistant protecting Malaysians from scams. If the user provides a suspicious phone number, ALWAYS use the 'querySemakmuleDB' tool to check it. If the user wants to report a scam, ALWAYS use the 'draftPoliceReport' tool to help them format it correctly. Use Markdown formatting. Be polite and professional.",
    tools: [
  ...
  const chat = model.startChat({
    ...
    // Everything except the latest message
  });
  ...
  if (functionCalls && functionCalls.length > 0) {
    ...
    if (functionName === "querySemakmuleDB") {
      apiResponse = await toolsFunctions.querySemakmuleDB(functionArgs);
    } else if (functionName === "draftPoliceReport") {
      apiResponse = await toolsFunctions.draftPoliceReport(functionArgs);
    } else {
